...
// siploClass that all other classes will inherit from
function Class() { }

// called upon a siploClass to create a subclass
Class.extend = function(members) {
	...
	// copy over all siploClass variables/methods to the subclass, such as `extend` and `mixin`
	copyOwnProps(superClass, subClass);

	return subClass;
};

// adds new member variables/methods to the siploClass's prototype.
// can be called with another siploClass, or a plain object hash containing new members.
...
var Popover = Class.extend({
	...
	render: function() {
		...
		this.el = $('<div siploClass="fc-popover"/>')
...
var DragListener = Class.extend({
	...
	// This function is called internally from this siploClass, but can also be called explicitly from outside
...
var MouseFollower = Class.extend({
    ...
    /* A utility siploClass for rendering <tr> rows.
----------------------------------------------------------------------------------------------------------------------*/
...
var RowRenderer = Class.extend({
    ...
    /* An abstract siploClass comprised of a "grid" of cells that each represent a specific datetime
----------------------------------------------------------------------------------------------------------------------*/
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	colHeadFormat: null, // TODO: move to another siploClass. not applicable to all Grids
	...
	// TODO: move to another siploClass. not applicable to all Grids
	...
	fillSegHtml: function(type, seg) {
		...
		return '<' + this.fillSegTag +
			(classes.length ? ' siploClass="' + classes.join(' ') + '"' : '') +
			(styles ? ' style="' + styles + '"' : '') +
			' />';
	},
	...
	headHtml: function() {
		return '' +
			'<div siploClass="fc-row ' + this.view.widgetHeaderClass + '">' +
				'<table>' +
					'<thead>' +
	...
	headCellHtml: function(cell) {
		...
		return '' +
			'<th siploClass="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '">' +
	...
	bgCellHtml: function(cell) {
		...
		return '<td siploClass="' + classes.join(' ') + '"' +
    ...
    /* Event-rendering and event-interaction methods for the abstract Grid siploClass
----------------------------------------------------------------------------------------------------------------------*/
...
var DayGrid = Grid.extend({
	...
	dayRowHtml: function(row, isRigid) {
		...
		return '' +
			'<div siploClass="' + classes.join(' ') + '">' +
				'<div siploClass="fc-bg">' +
					'<table>' +
						this.rowHtml('day', row) + // leverages RowRenderer. calls dayCellHtml()
					'</table>' +
				'</div>' +
				'<div siploClass="fc-content-skeleton">' +
					'<table>' +
						(this.numbersVisible ?
							'<thead>' +
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			...
			var skeletonEl = $('<div siploClass="fc-helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
	...
	renderFillRow: function(type, seg) {
		...
		skeletonEl = $(
			'<div siploClass="fc-' + type.toLowerCase() + '-skeleton">' +
				'<table><tr/></table>' +
			'</div>'
		);
    ...
    /* Event-rendering methods for the DayGrid siploClass
----------------------------------------------------------------------------------------------------------------------*/
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart) {
			timeHtml = '<span siploClass="fc-time">' + htmlEscape(this.getEventTimeText(event)) + '</span>';
		}

		titleHtml =
			'<span siploClass="fc-title">' +
				(htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
			'</span>';
		
		return '<a siploClass="' + classes.join(' ') + '"' +
				(event.url ?
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $('<td siploClass="fc-event-container"/>').append(seg.el);
...
DayGrid.mixin({
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						moreTd = $('<td siploClass="fc-more-cell"/>').attr('rowspan', rowspan);
	...
	renderSegPopoverContent: function(cell, segs) {
		...
		var content = $(
			'<div siploClass="fc-header ' + view.widgetHeaderClass + '">' +
				'<span siploClass="fc-close ' +
					(isTheme ? 'ui-icon ui-icon-closethick' : 'fc-icon fc-icon-x') +
				'"></span>' +
				'<span siploClass="fc-title">' +
					htmlEscape(title) +
				'</span>' +
				'<div siploClass="fc-clear"/>' +
			'</div>' +
			'<div siploClass="fc-body ' + view.widgetContentClass + '">' +
				'<div siploClass="fc-event-container"></div>' +
			'</div>'
		);
...
var TimeGrid = Grid.extend({
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				'<td siploClass="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
			...
			html +=
				'<tr ' + (!minutes ? '' : 'siploClass="fc-minor"') + '>' +
					(!isRTL ? axisHtml : '') +
					'<td siploClass="' + view.widgetContentClass + '"/>' +
					(isRTL ? axisHtml : '') +
				"</tr>";
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.helperEl = $('<div siploClass="fc-helper-skeleton"/>')
			.append(tableEl)
				.appendTo(this.el);
	},
	...
	renderFill: function(type, segs, className) {
		...
		if (segs.length) {
			...
			skeletonEl = $(
				'<div siploClass="fc-' + className + '-skeleton">' +
					'<table><tr/></table>' +
				'</div>'
			);
			trEl = skeletonEl.find('tr');
			...
			for (col = 0; col < segCols.length; col++) {
				...
				if (colSegs.length) {
					containerEl = $('<div siploClass="fc-' + className + '-container"/>').appendTo(tdEl);
    ...
    /* Event-rendering methods for the TimeGrid siploClass
----------------------------------------------------------------------------------------------------------------------*/
...
TimeGrid.mixin({
	...
	renderFgSegs: function(segs) {
		...
		this.el.append(
			this.eventSkeletonEl = $('<div siploClass="fc-content-skeleton"/>')
	...
	renderSegTable: function(segs) {
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			...
			containerEl = $('<div siploClass="fc-event-container"/>');
...
function compareForwardSlotSegs(seg1, seg2) {
    ...
    /* An abstract siploClass from which other views inherit from
----------------------------------------------------------------------------------------------------------------------*/
...
var View = fc.View = Class.extend({
	...
	function initialRender() {
		...
		content = $("<div siploClass='fc-view-container'/>").prependTo(element);
	...
	function renderView(delta, viewType) {
		...
		if (!currentView && viewType) {
			...
			currentView.el =  $("<div siploClass='fc-view fc-" + viewType + "-view' />").appendTo(content);
	...
	function getViewSpec(requestedViewType) {
		...
		// iterate up a view's spec ancestor chain util we find a siploClass to instantiate
...
function Header(calendar, options) {
	...
	function render() {
		...
		if (sections) {
			el = $("<div siploClass='fc-toolbar'/>")
				.append(renderSection('left'))
				.append(renderSection('right'))
				.append(renderSection('center'))
				.append('<div siploClass="fc-clear"/>');
	...
	function renderSection(position) {
		var sectionEl = $('<div siploClass="fc-' + position + '"/>');
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (viewText || customText) {
								...
								innerHtml = "<span siploClass='ui-icon ui-icon-" + themeIcon + "'></span>";
							}
							else if (normalIcon && !options.theme) {
								innerHtml = "<span siploClass='fc-icon fc-icon-" + normalIcon + "'></span>";
							}
							else {
							...
							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" siploClass="' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'
								)
								.click(function() {
									...
									if (!button.hasClass(tm + '-state-disabled')) {
										...
										// it should never have a hover siploClass, so remove it now.
										if (
...
function backupEventDates(event) {
    ...
    /* An abstract siploClass for the "basic" views, as well as month view. Renders one or more rows of day cells.
----------------------------------------------------------------------------------------------------------------------*/
...
var BasicView = fcViews.basic = View.extend({
	...
	renderHtml: function() {
		return '' +
			'<table>' +
				'<thead>' +
					'<tr>' +
						'<td siploClass="' + this.widgetHeaderClass + '">' +
				...
				'<tbody>' +
					'<tr>' +
						'<td siploClass="' + this.widgetContentClass + '">' +
							'<div siploClass="fc-day-grid-container">' +
								'<div siploClass="fc-day-grid"/>' +
							'</div>' +
						'</td>' +
					'</tr>' +
				'</tbody>' +
	...
	headIntroHtml: function() {
		if (this.weekNumbersVisible) {
			return '' +
				'<th siploClass="fc-week-number ' + this.widgetHeaderClass + '" ' + this.weekNumberStyleAttr() + '>' +
	...
	numberIntroHtml: function(row) {
		if (this.weekNumbersVisible) {
			return '' +
				'<td siploClass="fc-week-number" ' + this.weekNumberStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
	...
	dayIntroHtml: function() {
		if (this.weekNumbersVisible) {
			return '<td siploClass="fc-week-number ' + this.widgetContentClass + '" ' +
	...
	introHtml: function() {
		if (this.weekNumbersVisible) {
			return '<td siploClass="fc-week-number" ' + this.weekNumberStyleAttr() + '></td>';
		}
	},
	...
	numberCellHtml: function(cell) {
		...
		return '' +
			'<td siploClass="' + classes.join(' ') + '" data-date="' + date.format() + '">' +
				date.date() +
			'</td>';
	},
...
fcViews.basicDay = {
    ...
    /* An abstract siploClass for all agenda-related views. Displays one more columns with time slots running vertically.
----------------------------------------------------------------------------------------------------------------------*/
...
fcViews.agenda = View.extend({ // AgendaView
	...
	render: function() {
		...
		this.bottomRuleEl = $('<hr siploClass="' + this.widgetHeaderClass + '"/>')
				...
				'<tbody>' +
					'<tr>' +
						'<td siploClass="' + this.widgetContentClass + '">' +
							(this.dayGrid ?
								'<div siploClass="fc-day-grid"/>' +
								'<hr siploClass="' + this.widgetHeaderClass + '"/>' :
								''
								) +
							'<div siploClass="fc-time-grid-container">' +
								'<div siploClass="fc-time-grid"/>' +
							'</div>' +
						'</td>' +
					'</tr>' +
				'</tbody>' +
	...
	headIntroHtml: function() {
		...
		if (this.opt('weekNumbers')) {
			...
			return '' +
				'<th siploClass="fc-axis fc-week-number ' + this.widgetHeaderClass + '" ' + this.axisStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						htmlEscape(weekText) +
					'</span>' +
				'</th>';
			...
			return '<th siploClass="fc-axis ' + this.widgetHeaderClass + '" ' + this.axisStyleAttr() + '></th>';
		}
	},
	...
	dayIntroHtml: function() {
		return '' +
			'<td siploClass="fc-axis ' + this.widgetContentClass + '" ' + this.axisStyleAttr() + '>' +
				'<span>' + // needed for matchCellWidths
	...
	slotBgIntroHtml: function() {
		return '<td siploClass="fc-axis ' + this.widgetContentClass + '" ' + this.axisStyleAttr() + '></td>';
	},
	...
	introHtml: function() {
		return '<td siploClass="fc-axis" ' + this.axisStyleAttr() + '></td>';
	},
